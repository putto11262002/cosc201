 ...
 */
public class PriorityWarehouse extends Warehouse{
    public PriorityWarehouse(int rows, int cols){
        super(rows, cols);
    }
    public PriorityWarehouse(){super();}
                ...
                for(int c = 0; c < cols; c++){
                    ...
                    if(movedPacket != null){
                        if(movedPacket.current.equals(movedPacket.destination)){
                            deliveredPackets++;
                            System.out.println("Packet " + movedPacket.ID + " has been delivered at " + movedPacket.current);
